...
process.stdin.on("end", () => {
  try {
    ...
    if (/\bterraform\s+apply\b/.test(cmd)) {
      notes.push("Terraform apply was invoked. Confirm the final CLI status and remote state before treating the change as complete. Use `/gcp-devkit:terraform-gcp` to extend the module with additional resources.");
    }
    if (/\bgcloud\s+run\s+deploy\b/.test(cmd)) {
      notes.push("Cloud Run deploy was invoked. Confirm the final CLI status, then tail logs with `gcloud run services logs read <service> --region=<region> --limit=50`. If 500s appear, `/gcp-devkit:gcloud-debug cloud-run <service>` pulls the full diagnostic.");
    }
