...
description: Use for an explicitly requested multi-step migration plan to GCP from AWS or Azure. It maps services and creates a phased plan across infrastructure, networking, identity, data, and cutover. Do not invoke for one-shot Terraform generation or a single-service deployment; those fit regular skills.
model: sonnet
