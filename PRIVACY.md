# Privacy policy: gcp-devkit (Claude Code plugin)
...
A developer plugin for **Google Cloud Platform** workflows (for example Terraform, Cloud Run, GKE, Cloud Functions, and IAM guidance).
...
**Session hooks:** Optional Node.js scripts under `hooks/` may read files in your **current working directory** (for example to detect GCP-related project markers) and emit short text as **additional context** inside your Claude Code session. That runs locally where Claude Code is installed. This plugin does not send that output to a separate server operated by the plugin authors.
...
Please use the repository issue tracker for support, bug reports, and privacy questions.
