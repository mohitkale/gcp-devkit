...
- Self-hosted Claude Code marketplace metadata for GitHub distribution.
- Docker Desktop validation coverage for both hooks, JavaScript syntax, and manifests.
- A user-focused installation and safety guide with macOS and Windows ZIP instructions.

### Changed

- Reworked the README into a product guide with workflow and validation visuals.
- Rephrased state-changing hook follow-ups so a command is never reported as successful without verification.
- IAM audit guidance now reads policy output directly and does not write an `iam-policy.json` file into the user's project.
- Migration planning now requires an explicit request, matching the documented opt-in behavior.

### Security

- Removed the personal email address from distributable metadata, privacy text, and license text.
